...
async function trackSupabasePageView(meta = {}) {
  try {
    if (window.supabaseClient) {
      ...
      const { data, error } = await window.supabaseClient.trackEvent('page_view', trackingData);
      if (error) {
        console.warn('📊 Page view track failed due to Supabase error:', error);
        return;
      }
      console.log('📊 Page view tracked:', window.location.pathname, data);
    }
  } catch (error) {
